...
const getProjects = async ({user = '', admin = false} = {}) => {
	...
	try {
		...
		try{
			...
			const getQuery = {
				text: `SELECT p.id, p.title, p.presale_price, p.public_price, p.ts_presale_start, p.max_supply, p.website_url, p.website_url, p.twitter_url, p.discord_url, p.wl_register_url, p.score, p.upvotes, p.downvotes, p.is_active, p.from_syndicate,
